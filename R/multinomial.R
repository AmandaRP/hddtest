...
#' testing \eqn{H_0:} the underlying multinomial probability vectors are the same
...
#' Three different experiments with different probability vectors (of length \eqn{k}) are available in addition to user-specified probability vector \code{p}:
#' \itemize{
...
#' \item Experiment 3: \eqn{p_{1i} = 1/k}. When the \code{null_hyp} parameter is FALSE, \eqn{p_{2i} = 0} for \eqn{i \in 1...b} and \eqn{p_{2i} = 1/(k − b)}  for \eqn{i > b}.
#' }
#'
...
#' If defined, all remaining parameters in the function definition are ignored. Default value is NULL.
